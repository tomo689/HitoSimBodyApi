...
export const pancreasModel: OrganModel = {
  ...
  simulate(context: SimulationContext) {
    ...
    for (let i = 0; i < stepCount; i++) {
      ...
      const dG = (-p1 * (G - Gb) - X * G + mealPulse - exerciseEffect) * dtHours;
      const insulinRelease =
        p3 * Math.max(0, G - Gb) * 100 * dtHours;
    ...
    return {
      ...
      metrics: [
        ...
        { name: 'インスリン感受性 SI', value: round(si, 4), unit: '10^-5/min/(μU/mL)' },
